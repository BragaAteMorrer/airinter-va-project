...
public sealed class AcarsWindow : Window
{
    ...
    private async Task RefreshAsync() { if (ticking) return; ticking = true; try { await telemetry.Tick(); var f=recorder.Flight; status.Text = $"{sim.Status}   |   {(client.Connected ? client.Server : "phpVMS déconnecté")}   |   Phase : {f?.Phase ?? "-"}   |   Tampon : {recorder.Pending.Count + recorder.PendingEvents.Count + recorder.PendingFacts.Count}"; } finally { ticking = false; } }
}
