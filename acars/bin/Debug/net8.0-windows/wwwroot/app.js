...
function renderOperations(data) {
  ...
  operations.forEach(op => {
    ...
    card.className = 'flight'; card.type = 'button';
    const sb = op.simbrief || {}, rules = op.operating_rules || {};
    card.innerHTML = `<strong>${f.ident || 'Vol réservé'} · ${f.departure || '?'} → ${f.arrival || '?'}</strong>` +
      `<span>${op.aircraft?.subfleet || 'Avion à sélectionner'} · SimBrief ${sb.type || 'à configurer'}${sb.addon ? ` · ${sb.addon}` : ''}</span>` +
      `<small>${sb.available ? 'OFP disponible' : 'OFP à générer'} · remplissage ${rules.load_factor_percent ?? '-'} %</small>`;
    card.onclick = () => selectOperation(op);
    $('#flightList').append(card);
  });
  if (!operations.length) show($('#flightList'), 'Aucune réservation active pour ce pilote.');
}

function selectOperation(op) {
  const f = op.flight || {}, a = op.aircraft || {}, sb = op.simbrief || {}, rules = op.operating_rules || {};
  const form = $('#prefileForm');
  form.flight_id.value = f.id || ''; form.airline_id.value = f.airline_id || '';
  form.flight_number.value = f.flight_number || ''; form.dpt_airport_id.value = f.departure || '';
  form.arr_airport_id.value = f.arrival || '';
  if (a.id) {
    $('#aircraftId').innerHTML = `<option value="${a.id}">${a.registration || a.subfleet || 'Avion réservé'}</option>`;
    $('#aircraftId').value = String(a.id);
  }
  flightPlan = {name: 'Contexte opérationnel chargé', prefile: {
    ...(f.alternate && {alt_airport_id:f.alternate}), ...(f.route && {route:f.route}), ...(f.level && {level:Number(f.level)})
  }};
  show($('#planBox'), {reservation: op.bid_id, simbrief: sb, operating_rules: rules, message: sb.available ? 'OFP SimBrief déjà disponible.' : 'OFP SimBrief à générer pour cette réservation.'});
}
