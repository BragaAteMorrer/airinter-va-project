<div class="rule-builder" data-rule-builder>
    ...
    <template data-rule-template>
        <div class="rule-row" data-rule-row>
            ...
            <label data-award hidden>Badge requis <select data-award-value>@foreach($awards as $award)<option value="{{ $award->id }}">{{ $award->name }}</option>@endforeach</select></label>
            <label data-airline hidden>Compagnie <select data-airline-value>@foreach($airlines as $airline)<option value="{{ $airline->id }}">{{ $airline->icao }} · {{ $airline->name }}</option>@endforeach</select></label>
            <label data-aircraft hidden>Appareil <select data-aircraft-value>@foreach($aircraftTypes as $type)<option value="{{ $type }}">{{ $type }}</option>@endforeach</select></label>
            <label data-event hidden>Événement <select data-event-value>@foreach($events as $event)<option value="{{ $event->id }}">{{ $event->title }} · {{ $event->starts_at }}</option>@endforeach</select></label>
