...
<section class="panel"><div class="panel-heading"><div><span class="eyebrow">{{ strtoupper($key) }}</span><h2>{{ $title }}</h2></div><p>{{ $description }}</p><a class="button outline" href="{{ route('promethee.downloads.category',$key) }}">Ouvrir la page dédiée</a></div><div class="route-list">@forelse($groups->get($key, collect())->take(3) as $file)<article><strong>{{ $file->name }}</strong>@if($file->description)<span>{{ $file->description }}</span>@endif<a class="button outline" href="{{ route('promethee.downloads.download', $file->id) }}">Télécharger</a></article>@empty<p class="empty">Aucune ressource disponible dans cette catégorie.</p>@endforelse</div></section>
@endforeach
@endsection
