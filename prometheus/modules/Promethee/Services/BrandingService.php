...
class BrandingService
{
    ...
    public function active(): array
    {
        ...
        if (str_starts_with($key, 'custom/') && is_file(public_path('promethee-assets/logos/'.$key))) {
            return [
                'key' => $key,
                'label' => 'Logo importé',
                'file' => $key,
                'description' => 'Logo personnalisé importé par l’administration.',
                'url' => asset('promethee-assets/logos/'.$key),
            ];
        }
    ...
    public function saveCustom(string $file): void
    {
        $this->save('custom/'.$file);
    }
}
