(() => {
  ...
  const update=()=>{const locked=root.dataset.loginEra==='minitel'&&!valid();controls.forEach(control=>control.disabled=locked);external.forEach(link=>link.hidden=locked);if(status){status.textContent=locked?'COMPOSEZ 3615 AIR INTER POUR OUVRIR LE SERVICE.':'SERVICE OUVERT. IDENTIFIEZ-VOUS.';status.style.color=locked?'#f0f':'#0f0'}};
