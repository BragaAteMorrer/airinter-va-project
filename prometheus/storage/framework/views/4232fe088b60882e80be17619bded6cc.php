<?php $__env->startSection('scripts'); ?>
  <script>
    const changeStatus = async (values, fn) => {
      ...
      const opts = {
        method: 'POST',
        url: '<?php echo e(url('/admin/pireps')); ?>/' + values.pirep_id + '/status',
        data: values,
      };
    ...
    $(document).ready(() => {
      ...
      $(select_id).change(async (e) => {
        ...
        const response = await phpvms.request("<?php echo e(url('/admin/pireps/fares')); ?>?aircraft_id=" + aircraft_id);
      ...
      $('button#recalculate-finances').on('click', async (event) => {
        ...
        const opts = {
          method: 'POST',
          url: '<?php echo e(url('/api/pireps')); ?>/' + pirep_id + '/finances/recalculate',
        };
      ...
      $(document).on('submit', 'form.pirep_submit_status', (event) => {
        ...
        changeStatus(values, (data) => {
          ...
          if (values.new_status === '<?php echo e(App\Models\Enums\PirepState::ACCEPTED); ?>') {
            new_badge = 'badge badge-success';
            new_badge_text = 'Accepted'
          }

          if (values.new_status === '<?php echo e(App\Models\Enums\PirepState::REJECTED); ?>') {
...
<?php /**PATH /home/jewe0363/prometheus/resources/views/admin/pireps/scripts.blade.php ENDPATH**/ ?>